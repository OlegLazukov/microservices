...
from src.services.auth_service.utils.repository import AbstractRepository
from src.services.auth_service.utils.unit_of_work import AbstractUnitOfWork, UnitOfWork
...
class AbstractService(ABC):
    ...
    @abstractmethod
    async def verify_password(self, *args: Any, **kwargs: Any) -> Never:
        raise NotImplementedError
...
class BaseService(AbstractService):
    ...
    @staticmethod
    def check_auth(obj: Any, details: str) -> None:
        """Checking the existence of an object."""
        if not obj:
            raise HTTPException(status_code=HTTP_401_UNAUTHORIZED, detail=details)
